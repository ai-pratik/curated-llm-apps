...
with st.sidebar:
    ...
    ollama_model = st.selectbox(
        "Ollama Model",
        ["llama3.2:latest", "llama3:8b", "gemma3:1b"],
        help="llama3.2 recommended for tool calling"
    )
...
async def run_github_agent(message, model_id):
...
if st.button("🚀 Run Query", type="primary", use_container_width=True):
    if not github_token:
        ...
        with st.spinner(f"Analyzing GitHub repository using {ollama_model}..."):
            if repo and repo not in query:
                full_query = f"{query} in {repo}"
            else:
                full_query = query

            result = asyncio.run(run_github_agent(full_query, ollama_model))
...
if 'result' not in locals():
    st.markdown(
        ...
        <ol>
            <li>Select your <strong>Ollama model</strong> in the sidebar (llama3.2 recommended)</li>
        ...
        <ul>
            ...
            <li>AI Agent (powered by local Ollama) interprets your queries and calls appropriate GitHub APIs</li>
